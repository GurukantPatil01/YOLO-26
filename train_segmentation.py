...
def main():
    ...
    h = int(((540 / 2) // 14) * 14)
    lr = 1e-4
    n_epochs = 30  # increased for better results
    ...
    # Dataset paths — edit these to match where you extracted the dataset
    # Option 1: dataset is sibling of project folder (official layout)
    data_dir = os.environ.get('TRAIN_DIR', os.path.join(script_dir, '..', 'Offroad_Segmentation_Training_Dataset', 'train'))
    val_dir  = os.environ.get('VAL_DIR',   os.path.join(script_dir, '..', 'Offroad_Segmentation_Training_Dataset', 'val'))
    # Option 2: dataset inside project data/ folder
    # data_dir = os.path.join(script_dir, 'data', 'train')
    # val_dir  = os.path.join(script_dir, 'data', 'val')
    print(f"Train data: {os.path.abspath(data_dir)}")
    print(f"Val data:   {os.path.abspath(val_dir)}")
